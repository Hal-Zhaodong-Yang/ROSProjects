...
class maze_node:
    def __init__(self):
        ...
        self.rangeOfViewForStopping = 10 # Check from 10 degrees to -10 degrees for a wall
    ...
    def lidarCallback(self, ros_data):
        ...
        if not self.odometryComplete:
            ...
            vel = Twist()
            if abs(self.globalAng - self.initialDesiredAngleRad) > 3*math.pi/180:
                vel.linear.x = 0
                vel.linear.y = 0
                vel.linear.z = 0
                vel.angular.x = 0
                vel.angular.y = 0
                vel.angular.z = self.turnDirection*math.pi/36 # 5 degrees per second
            else:
                vel.linear.x = 0
                vel.linear.y = 0
                vel.linear.z = 0
            ...
            if self.closestAngDegrees != 0:
                ...
                vel.angular.x = 0
                vel.angular.y = 0
                vel.angular.z = self.sign(self.closestAngDegrees)*math.pi/180 # 1 degree per second
            else:
                vel.linear.x = 0
                vel.linear.y = 0
                vel.linear.z = 0
...
def main(args):
    flag = 1
    rospy.init_node('maze_node', anonymous=True)
    if flag == 1:
        ic = maze_node()
        flag = 0
