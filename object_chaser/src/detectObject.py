...
lower= np.array([99,182,72],np.uint8) 
upper= np.array([113,232,163],np.uint8)
...
error = np.array([6,5,5],np.uint8) # Array of error widths to create the upper and lower threshold bounds above.
...
def mouseEvent(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if is_first_obj:
            ...
            for i in range(3):
                lower_rec[i] = min([lower_rec[i], lower[i]])
                upper_rec[i] = max([upper_rec[i], upper[i]])
	...
	check_lower = np.uint16(originalH - error[0])
        ...
        rospy.loginfo("Hue Range Expanded: [%d  %d]",lower_rec[0], upper_rec[0])
        rospy.loginfo("Sat Range Expanded: [%d  %d]",lower_rec[1], upper_rec[1])
        rospy.loginfo("Value Range Expanded: [%d  %d]",lower_rec[2], upper_rec[2])
...
def Init():
    ...
    global pub
    pub = rospy.Publisher('/geometry_msgs/Point', Point, queue_size=10)
